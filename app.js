...
setInterval(async () => {
    try {
        ...
        for (const item of expiringAccounts) {
            const { chatId, account } = item;
            
            const message = `
⚠️ <b>תזכורת לגבי החשבון שלך</b>
...
💡 אם תרצה להמשיך ליהנות מהשירות, אפשר ליצור חשבון חדש.
            `;
...
❌ חשבונות לא פעילים: <b>${expiredAccounts}</b>
...
bot.onText(/\/myaccounts/, async (msg) => {
    ...
    accounts.forEach((acc, idx) => {
        ...
        const statusText = acc.active ? '✅ פעיל' : '❌ לא פעיל';
        ...
        if (!limitCheck.allowed) {
            ...
            if (userAccounts.length > 0) {
                ...
                userAccounts.forEach((acc, idx) => {
                    const status = acc.active ? '✅ פעיל' : '❌ לא פעיל';
